...
@section('namecontent','Edit Postingan')
...
<section class="content">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{route('postingan.update', $datapost->id)}}" method="post" class="form-group" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                <input type="hidden" id="id_user" name="id_user" value="{{auth::user()->id}}">
                <div class="row">
                    <div class="col-md-6">
                        <img id="preview" src="{{URL('/')}}/asset/imagepost/{{$datapost->gambar}}"  width="400" height="auto" />
                        <input type="hidden" name="hidden_image" id="hidden_image" value="{{$datapost->gambar}}">
                        <hr>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            <input type="text" class="form-control" id="title" name="title" value="{{$datapost->title}}" required placeholder="Title Post">
                        </div>
                        <br>
                        <div class="input-group">
                            <textarea name="diskripsi" id="diskripsi" class="form-control" cols="30" rows="4" required placeholder="Deskripsi Postingan">{{$datapost->diskripsi}}</textarea>    
                        </div>
                        <br>
                        <div class="row">
                            <div class="col-md-6">
                                <div class="input-group">
                                    <select name="id_label" id="id_label" class="form-control" required>
                                        @if ($label->count()>0)
                                            ...
                                            @foreach ($label as $l)
                                                <option value="{{$l->id}}" @if($datapost->id_label) selected @endif>{{$l->label}}</option>
                                            @endforeach
                                        @else
                                    ...
                                    <select name="id_tag" id="id_tag" class="form-control" required>
                                        @if ($tags->count()>0)
                                            ...
                                            @foreach ($tags as $t)
                                                ...
                                                <option value="{{$t->id}}"  @if($datapost->id_tag) selected @endif>{{$t->tag}}</option>
                                            @endforeach
                                        @else
                                        <option value="" disabled selected>Kosong Njing</option>
                                        @endif
